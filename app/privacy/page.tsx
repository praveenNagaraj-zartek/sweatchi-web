  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const response = await fetch(`${BASE_URL}/content/pages/privacy/`);

        if (!response.ok) {
          throw new Error(
            `Failed to fetch privacy page: ${response.statusText}`
          );
        }
        ...
        <div className="sm:glassmorphism sm:rounded-2xl p-4">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-white text-lg">Loading privacy policy...</p>
            </div>
          ) : error ? (
            <div className="text-center py-12">
              <p className="text-red-400 text-lg mb-4">{error}</p>
              <p className="text-white leading-relaxed">
                Unable to load privacy policy content. Please try again later.
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="prose prose-invert max-w-none">
                {pageContent?.content ? (
                  ...
                  <p className="text-white leading-relaxed">
                    Unable to load privacy policy content. Please try again
                    later.
                  </p>
                )}
              </div>
